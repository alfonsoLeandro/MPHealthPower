...
public class ConsumableSetHandler extends AbstractHandler {

    private final ConsumableManager consumableManager;

    public ConsumableSetHandler(HealthPower plugin, AbstractHandler successor) {
    ...
    protected void internalHandle(CommandSender sender, String label, String[] args) {
        if (!sender.hasPermission("HealthPower.consumable.set")) {
        ...
        this.messageSender.send(sender, Message.CONSUMABLE_CREATED,
                "%name%", consumableName,
                "%mode%", mode,
